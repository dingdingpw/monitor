...
<body>
  ...
  <div id="panel" class="shell hidden">
    ...
    <main class="main">
      ...
      <div class="grid">
        <section class="card"><h3>添加节点</h3><div class="row"><input id="nodeId" placeholder="US-node-001"><button onclick="addNode()">添加并生成</button><button class="secondary" onclick="loadNodes()">刷新</button></div><p class="muted">Node ID 必须唯一，建议前两位使用国家或地区代码。</p></section>
